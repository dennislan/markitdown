...
Resolves the Python runtime in priority order: (1) the app-bundle embedded runtime at `Contents/Resources/python/markitdown-env` (produced by the "Embed Python Runtime" build phase / `build_release.sh`), (2) the repo-local `markitdown-env` derived from `#filePath` during development — no machine-specific absolute paths remain. The Python version is read from `pyvenv.cfg`, not hardcoded. The proxy constructs inline Python scripts that `sys.path.insert()` the site-packages, import `markitdown.MarkItDown`, and call `.convert()`. Output is captured from subprocess stdout; errors from stderr. For `.ppt` files, a custom `PptConverter.py` source is written to a temp dir and registered via a `_RegMarkItDown` subclass (env `MARKITDOWN_PPT_CONVERTER_PATH`). PDFs run through the default markitdown converter; afterwards `postProcessPDFResult(_:filePath:)` inspects the output — if it detects encoding-corrupted extraction (empty text, `(cid:NNN)` placeholders, U+FFFD, or control characters), it falls back to on-device OCR via `ocrPDFWithVision(filePath:)` (PDFKit page thumbnails → Vision `VNRecognizeTextRequest`, `zh-Hans` + `en-US`). No Python OCR stack is embedded in the venv; `onnxruntime` remains only because `markitdown`'s top-level `magika` import requires it.
